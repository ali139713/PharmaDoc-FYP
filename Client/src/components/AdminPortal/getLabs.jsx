...
const Lab = (props) => (
  <tr>
    <td className={props.lab.todo_completed ? "completed" : ""}>
      {props.lab.name}
    </td>
    <td className={props.lab.todo_completed ? "completed" : ""}>
      {<img style={imageStyle} src={props.lab.labImage} alt="img" />}
    </td>
    <td className={props.lab.todo_completed ? "completed" : ""}>
      {props.lab.status}
    </td>
    <td>
      {" "}
      <Link to={`/admin/labs/updateLabs/${props.lab._id}`}>
        {" "}
    ...
    <td>
      <i
        ...
        onClick={() => props.handleDelete(props.lab)}
      ></i>
    </td>
  </tr>
);
  ...
  handleDelete = async (obj) => {
    ...
    try {
      ...
      const response = await Axios.delete(
        "http://localhost:5000/lab/delete/" + obj._id
      );
    } catch (error) {
      console.log(error);
        ...
        <table className="table table-striped" style={{ marginTop: 20 }}>
          ...
          <tbody style={{ overflowY: "auto" }}>
            {this.state.labs.map((currentlab, i) => (
              <Lab lab={currentlab} key={i} handleDelete={this.handleDelete} />
            ))}
          </tbody>
        </table>
      </div>
